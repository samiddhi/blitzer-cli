...
** Available Languages
- =base= :: Basic processor for space-separated languages (no lemmatization support); 'generic' is equivalent but not listed separately
...
- =base= :: Basic processor for space-separated languages without lemmatization (always available); 'generic' is equivalent but not listed separately
